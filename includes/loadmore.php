...
$slug_url=$_POST['url'];
$page=$_POST['page'];
$row=($page-1)*($post_per_page);
$pquery = "SELECT * FROM posts WHERE cat_id={$cat_id} LIMIT $row, $post_per_page";
$prun = mysqli_query($conn, $pquery) or die("Query Failed");
while ($post = mysqli_fetch_assoc($prun)) {
    list($width, $height, $type, $attr) = getimagesize("../thumbnail/{$post['thumbnail']}");
    if ($post['slug_url'] == $slug_url) {
        continue;
    }
    // ------------------------------Box Section  ------------------ -->?>
