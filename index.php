...
if (isset($_SERVER['PATH_INFO'])) {
  $slug_url = ltrim($_SERVER['PATH_INFO'], "/");
  $postQuery = "SELECT * FROM posts WHERE slug_url='$slug_url'";
  $runPQ = mysqli_query($conn, $postQuery);
  $num = mysqli_num_rows($runPQ);
  if (mysqli_num_rows($runPQ) <= 0) {
    header('location: 404.html');
  } else {
    $mpost = mysqli_fetch_assoc($runPQ);
    $title = $mpost['post_title'];
    $description = $mpost['post_desc'];
    $tags = $mpost['tags'];
  }
}
?>
...
<head>
  ...
  <?php if (isset($_SERVER['PATH_INFO'])) { ?>
    <meta name="description" content="<?= $title ?> free transparent background Clipart" />
    <meta name="keywords" content="<?= $tags ?>,clip art,studio background,animal png,flower png,free download" />
    <meta name="author" content="Pintu Kumar - PTS">
    <title><?= $title ?></title>
    <link rel="canonical" href="<?= $location . '/' . $slug_url ?>">

    <meta property="og:title" content="<?= $title ?>" />
    <meta property="og:image" content="<?= $location . '/' . 'thumbnail/' . $mpost['thumbnail'] ?>" />
    <meta property="og:url" content="<?= $location . '/' . $mpost['slug_url'] ?>" />
    <meta property="og:site_name" content="<?= $location ?>" />
  <?php } else { ?>
    <meta name="description" content="All Types PNG image free download | Stylish hindi font clipart PNG download, poster designing PNG, banner editing png, Free PNG images download" />
    <meta name="keywords" content="free PNG,transparent background,free clipart,clip art,studio background,animal png,flower png,free download" />
    <meta name="author" content="Pintu Kumar - PTS">
  ...
  <?php if (isset($_SERVER['PATH_INFO'])) {
    ...
    <section class="post">
      <div class="container-fluid shadow page-title">
        <h6 class="text-center "><?= $mpost['post_title'] ?></h6>
      </div>
      <div class="container">
        <div class="row px-lg-3 px-xl-5  gx-4 ">
          <?php
          $pngname = get_basename($mpost['thumbnail']) . '.png';
          $info = getimagesize("png/$pngname");
          ?>
          <div class=" col-xl-9">
            <div class="shadow">
              <figure class=" bg-white">
                <img itemprop="contentUrl" alt="<?= $mpost['post_title'] ?>" title="<?= $mpost['post_title'] ?>" src="<?= $location ?>/thumbnail/<?= $mpost['thumbnail'] ?>" oncontextmenu="return false;" />
              ...
              <div class="social-share shadow">
                <ul class="info_list p-0 m-0">
                  <li><a target="_blank" href="https://twitter.com/share?text=<?= $mpost['post_title'] ?>&url=<?= $location . '/' . $mpost['slug_url'] ?>"><i class="fab fa-twitter-square" style="color: #03A9F4;"></i></a></li>

                  <li><a target="_blank" href="https://facebook.com/share.php?u=<?= $location . '/' . $mpost['slug_url'] ?>&p[title]=<?= $mpost['post_title'] ?>"><i class="fab fa-facebook-square" style="color: #3F51B5;"></i></a></li>

                  <li><a target="_blank" href="https://pinterest.com/pin/create/button/?url=<?= $location . '/' . $mpost['slug_url'] ?>&media=<?= $location . '/' . 'thumbnail/' . $mpost['thumbnail'] ?>&description=<?= $mpost['post_title'] ?>"><i class="fab fa-pinterest-square" style="color: #E60023;"></i></a></li>

                  <li><a target="_blank" href="https://api.whatsapp.com/send?text=<?php echo (urlencode($mpost['post_title'])); ?> <?= $location . '/' . $mpost['slug_url'] ?>"><i class="fab fa-whatsapp-square" style="color: #40C351;"></i></a></li>

                  <li><a target="_blank" href="https://www.tumblr.com/share/link?url=<?= $location . '/' . $mpost['slug_url'] ?>&amp;name=<?= $mpost['post_title'] ?>"><i class="fab fa-tumblr-square" style="color: #304D6A;"></i></a></li>
                </ul>
              </div>
              <button class="d-none" id="mainpost" data-id="<?=$mpost['cat_id']?>" data-slugurl="<?=$slug_url?>"></button>
              <div class=" bg-white pb-2">
                ...
                <ul class="tag_ul cat-ul p-0">
                  ...
                  foreach ($tags as $tag) {
                  ?>
                    <li><a href="search.php?search=<?= $tag ?>"><span rel="tag"><?= $tag ?></span></a></li>
                  <?php
                  } ?>
                </ul>
              </div>
            ...
            <div class="shadow bg-white pb-1">
              ...
              <ul class="info_list ps-2 mb-1">
                <li>
                  ...
                  <span class="info_detail"><?= $info[0] ?> x <?= $info[1] ?> px</span>
                </li>
                <li>
                  <span class="info_title">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                  ...
                  <span class="info_detail"><?= toConvert($file_size) ?></span>
                </li>
                <li>
                  <span class="info_title">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                  ...
                  <span class="info_detail">image/png</span>
                </li>
              ...
              <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-footer d-flex justify-content-between">
                    <a type="button" id="at-downloadx" href="<?= $location ?>/png/<?= get_basename($mpost['thumbnail']) ?>.png" download="<?= $location ?>/png/<?= get_basename($mpost['thumbnail']) ?>.png" class="btn btn-primary text-white">Download</a>
  ...
  <?php } else { ?>
    ...
    <div id="home">
      ...
      <header class="container-fluid home-page">
        <div class="row">
          <div class="col-lg-5 col-md-8 col-sm-7 m-auto">
            <a href="<?= $location ?>" class="logo d-inline-block"><img src="<?= $location ?>/images/logo/logo.png" alt="logo"></a>
            <form class="search-bar d-flex" action="search.php?=<?= $_GET['search'] ?>">
              <input class="px-3" type="search" required name="search" placeholder="Search for free png clipart..." />
              <button type="submit"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                  <path fill="#F14A2D" d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path>
                </svg></button>
            </form>
        ...
        <ul class="cat-ul ">
          ...
          foreach ($categories as $category) {
            if ($category['status'] == 1 && $category['no_of_posts'] > 0) {
              echo "<li><a href='?png={$category['cat_name']}'> {$category['cat_name']}</a></li>";
            }
          }
          ?>
        </ul>
      ...
      <div class="category-sm sticky-top dropdown w-100">
        <button class="btn w-100 btn-lg btn-light dropdown-toggle" type="button" style="border-bottom: 2px solid #dd5500;" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
          Category
        </button>
        <ul class="dropdown-menu w-100" aria-labelledby="dropdownMenuButton1">
          <li><a class="dropdown-item" href="<?= $location ?>">All</a></li>
          <?php
          $categories = getAllCategory($conn);
          foreach ($categories as $category) {
            if ($category['status'] == 1 && $category['no_of_posts'] > 0) {
              echo "<li><a class='dropdown-item' href='?png={$category['cat_name']}'> {$category['cat_name']}</a></li>";
            }
          }
          ?>
        </ul>
      ...
      <div class=" container-fluid gallery">
        <?php
        if (isset($_GET['png'])) {
          $keyword = mysqli_real_escape_string($conn, $_GET['png']);
          $postQuery1 = "SELECT cat_id FROM category WHERE cat_name='{$keyword}'";
          $runPQ1 = mysqli_query($conn, $postQuery1);
          $cat = mysqli_fetch_assoc($runPQ1);
          $postQuery = "SELECT * FROM posts WHERE cat_id={$cat['cat_id']} ORDER BY post_id DESC LIMIT $result,$post_per_page";
        } else {
          $postQuery = "SELECT * FROM posts ORDER BY post_id DESC LIMIT $result,$post_per_page";
        }
        $runPQ = mysqli_query($conn, $postQuery);
      ...
      <?php
      if (isset($_GET['png'])) {
        $q = "SELECT * FROM posts WHERE cat_id={$cat['cat_id']}";
      } else {
        $q = "SELECT * FROM posts";
      }
      ...
      <nav aria-label="Page navigation example" class="mt-4">
        <ul class="pagination justify-content-center">
          ...
          <li class="page-item <?= $switch ?>">
            <a class="page-link" href="?<?php if (isset($_GET['png'])) {
                                          echo "png=$keyword&";
